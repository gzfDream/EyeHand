...
bool Pointcloud_processing::show_pointCloud(){
	...
	while (!viewer.wasStopped() && loop)
	{
		...
		if (SUCCEEDED(hResult))
		{
			...
			if (key == '1')
			{
				cv::imwrite("../data/color/" + std::to_string(t1) + ".png", eximg);
				cv::imwrite("../data/depth/" + std::to_string(t1) + ".png", mat_depth);
...
bool Pointcloud_processing::getHDImage(){
	...
	while (loop){
		...
		if (key == '1')
		{
			cv::imwrite("../data/color_cam_cal/" + std::to_string(t1) + ".png", colorImg);
			t1++;
		}
		else if (key == 27){
			loop = false;
